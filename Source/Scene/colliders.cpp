...
bool intersects(Transform& transform, CircleCollider2D& collider, glm::vec2 pos) {
	return Math::Intersects(glm::vec2(transform.position) + collider.offset, collider.radius * transform.scale.x, pos);
}
...
bool intersects(Transform& transform, RectangleCollider2D& collider, glm::vec2 pos) {
	return Math::Intersects(glm::vec2(transform.position) + collider.offset, glm::vec2(collider.size.x * transform.scale.x * 2.0f, collider.size.y * transform.scale.y * 2.0f), pos);
}